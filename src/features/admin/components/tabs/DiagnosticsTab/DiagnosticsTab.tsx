...
import type { Diagnostics, DiagnosticKind, Resolution, ServerHealth } from '@/features/admin/model/types';
            ...
            <AdminCard title="Вызовы ИИ">
                <div className="permit-grid">
                    {diagnostics?.permits.map((permit) => (
                        <div className={`permit-grid__slot permit-grid__slot--${permit.state}`} key={permit.slot}>
                ...
                <p className="admin-card__help">Неопределённый вызов удерживает место до подтверждения завершения.</p>
            </AdminCard>
